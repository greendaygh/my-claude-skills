...
"워크플로 논문 마이닝", "paper mining", "논문 수집", "wf-paper-mining 실행", "재추출", "reextract"
...
├── registry_index.json             # 전체 워크플로 인덱스 (경량)
├── WB030/                          # 워크플로별 출력
│   ├── wf_state.json               # 워크플로별 상태 (v2)
│   ├── 01_papers/
│   │   ├── paper_list_1.json
│   │   └── full_texts/
│   │       └── WB030_P001.txt
...
>
> **도메인 특이성 검사**: 관대한 수용 원칙을 유지하되, 대상 워크플로({WF_ID})와 전혀 무관한 워크플로만 다루는 논문은 reject. 대상 워크플로가 upstream/downstream이라도 언급되면 accept.
>
> 출력 JSON 형식:
> ```json
> {
...
> → **반드시 extraction_status가 'pending'이고 has_full_text가 true인 논문만 처리.**
> → (주의: panel_b_verdict 필드가 아닌 extraction_status 필드로 필터링할 것. rejected 논문은 extraction_status가 'rejected'로 설정되어 있음.)
>
> 각 논문에 대해:
> 1. {wf_output_dir}/01_papers/full_texts/{paper_id}.txt를 읽는다 (대용량 파일은 limit 파라미터를 사용하여 분할 읽기)
...
>
>    **uo_connections 규칙**:
>    - 카탈로그에 존재하는 UO: catalog_id (예: UHW250, USW130)를 사용
>    - is_new=true인 UO: 해당 UO의 name을 사용
>    - 워크플로 이름이나 워크플로 ID를 사용하지 마십시오
>    - 워크플로 간 관계는 workflow_connections 필드에 별도 기록하십시오
>    - from_uo와 to_uo는 반드시 이 추출 파일의 hardware_uos 또는 software_uos에 존재하는 UO를 참조해야 합니다
>
>    **workflows 규칙**:
>    - workflows 배열에는 논문에서 식별된 실제 워크플로를 정확히 기록하십시오
>    - 처리 대상 워크플로({WF_ID})가 논문에 언급되어 있다면 반드시 배열에 포함할 것
>
>    **doi 규칙**:
>    - 추출 결과 최상위에 반드시 "doi" 필드를 포함하십시오
>    - paper_list에서 해당 논문의 doi를 복사합니다
...
JSON cleanup 후 상태 동기화:

```bash
python -m scripts.run_tracker sync-after-cleanup \
...
> - Round 1: 각 전문가가 독립적으로 score(0.0~1.0 float, 소수점 필수. 예: 0.85. 정수 불가) + notes/issues 작성
> - Round 2: 점수 공유 후 토론
...
사용자가 "재추출", "reextract" 키워드를 사용할 때 아래 체크리스트를 따른다.
기존 full text에서 개선된 프롬프트로 추출을 다시 수행한다. 검색(Phase 2), Panel B(Phase 3), full text fetch는 생략.
...
```bash
cd $SKILL_DIR && python -m scripts.list_reextract_targets \
  --wf-id {WF_ID} --root-dir $ROOT_DIR
```

출력의 `total_targets` 수를 확인하고 사용자에게 보고.
...
일반 모드 Phase 4의 배치 추출과 **동일한 프롬프트**를 사용하되, 필터링 조건만 다르다:

> "accept된 논문들의 full text를 순차적으로 읽고 추출한다.
>
> **재추출 모드**: paper_list 대신, {wf_output_dir}/01_papers/full_texts/ 디렉토리의 모든 .txt 파일을 대상으로 한다.
> 각 파일명에서 paper_id를 추출한다 (예: WB005_P006.txt → WB005_P006).
>
> 각 논문에 대해:
> 1. {wf_output_dir}/01_papers/full_texts/{paper_id}.txt를 읽는다 (대용량 파일은 limit 파라미터를 사용하여 분할 읽기)
> 2. extraction_template.json 스키마에 따라 추출:
...
>    - from_workflow와 to_workflow는 워크플로 catalog_id (예: WB045, WT010)를 사용
>    - relationship는 upstream / downstream / parallel 중 하나
>
>    **workflows 규칙**:
>    - workflows 배열에는 논문에서 식별된 실제 워크플로를 정확히 기록하십시오
>    - 처리 대상 워크플로({WF_ID})가 논문에 언급되어 있다면 반드시 배열에 포함할 것
...
>    **doi 규칙**:
>    - 추출 결과 최상위에 반드시 "doi" 필드를 포함하십시오
>    - wf_state.json의 paper_status에서 해당 논문의 doi를 참조합니다
> 3. 결과를 {wf_output_dir}/02_extractions/{paper_id}.json에 저장 (기존 파일 덮어쓰기)
> 4. 각 파일 저장 후 JSON 유효성을 검증. 실패 시 해당 파일 삭제 후 다음 논문으로 진행.
>
> **추출 범위**: 타깃 워크플로 + upstream/downstream 워크플로
> **주의**: 정보가 없는 str 필드는 null이 아닌 빈 문자열 ""을 사용할 것.
> UO 카탈로그: {extraction_guide}
...
```bash
cd $SKILL_DIR && python -m scripts.run_tracker sync-after-cleanup \
  --wf-id {WF_ID} --root-dir $ROOT_DIR
```

### 5. Panel C + 집계

Panel C와 aggregate는 일반 모드 Phase 5a, 5b와 동일하게 수행.

### 6. 검증

```bash
cd $SKILL_DIR && python -m scripts.validate_outputs \
  --output-dir $ROOT_DIR/{WF_ID} --verbose
```

### 7. 백업 정리

재추출 결과가 만족스러우면 백업 삭제:
```bash
